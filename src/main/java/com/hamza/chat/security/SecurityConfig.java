...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	auth.inMemoryAuthentication().withUser("hamza").password(passwordEncoder.encode("123")).roles("AGENT","USER");
	auth.inMemoryAuthentication().withUser("user1").password(passwordEncoder.encode("123")).roles("USER");*/
	 ...
	 protected void configure(HttpSecurity http) throws Exception {
		 ...
		 .antMatchers("/supprimerChat","/modifierChat","/updateChat")
		 .hasAnyRole("ADMIN");
	 ...
	 http.formLogin().loginPage("/login").defaultSuccessUrl("/listeChats");;
